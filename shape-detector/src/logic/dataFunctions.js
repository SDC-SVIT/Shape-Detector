...
export function calculateVertices(pointsArray) {
    ...
    let gradient_list = pointsArray.points.reduce(
        (grads_and_prev_point, current_point, current_index) => {
            ...
            let approx_current_grad =
                (current_point.y - grads_and_prev_point.prev_point.y) /
                (current_point.x - grads_and_prev_point.prev_point.x)
            let current_gradient = isNaN(approx_current_grad) ?
                last_grad != 0 && prev_grad_last_index + 1 != 0 ?
                last_grad :
                0 :
                approx_current_grad
                // let current_gradient = (current_point.y - grads_and_prev_point.prev_point.y) / (current_point.x - grads_and_prev_point.prev_point.x)
            return {
                gradients: [...grads_and_prev_point.gradients, current_gradient],
     ...
     */
    let gradient_trial = gradient_list.reduce(
        (prev_avg_gradients, current_gradient, current_index, all_gradients) => {
            if (current_index > 4 && current_index < all_gradients.length - 5) {
                let avg_grad = all_gradients.slice(current_index - 5, current_index + 5)
                    .reduce((accumulator, current_value) => accumulator + current_value);

                avg_grad = avg_grad / 10;
    ...
    let vertices_trial = gradient_trial.reduce(
        (vertices_and_prev_point, current_gradient, current_index) => {
            // TODO: Compute this again
            if (
                (current_gradient > 0 && vertices_and_prev_point.prev_point <= 0) ||
                (current_gradient < 0 && vertices_and_prev_point.prev_point >= 0)
            ) {
                vertices_and_prev_point.vertices.push(current_index + 5)
            }
            vertices_and_prev_point.prev_point = current_gradient
            return vertices_and_prev_point
        }, { vertices: [], prev_point: gradient_list[0] }
    ).vertices
    let return_obj = [
...
export function calculateMidPoints(point1, point2, point3) {
    ...
    let { m1, c1, m2, c2 } = calculateLineParamtersForVertice(
        point1,
        point2,
        point3
    )
    ...
    console.log('The Final set of midpoints are:')
